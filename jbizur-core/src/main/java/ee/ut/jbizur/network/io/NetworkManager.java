...
public class NetworkManager implements AutoCloseable, ResourceCloser {
    ...
    protected Multicaster createMulticaster() throws UnknownHostException {
        if (CoreConf.get().network.multicast.enabled) {
            MulticastAddress multicastAddress = new MulticastAddress(CoreConf.get().network.multicast.address);
    ...
    protected AbstractServer createServer() {
        try {
            return AbstractServer.create(
                    (Class<? extends AbstractServer>) Class.forName(CoreConf.get().network.server),
                    "server-" + name,
                    serverAddress
            );
